...
Built binaries for Windows/Linux/MacOSX can be found at <https://github.io.sked/docs/releases>
...
A wizard will ask you for the import configuration and write it in `~/.config/preferences/io.sked.docs.importer.pref`.
...
The docker image needs a volume mount of a previously generated preference file to `/root/.config/preferences/io.sked.docs.importer.pref`. The container will start the importer in daemon mode. It will look for files in `/import`.
Example usage:

```
docker build -t teedy-import .
docker run --name teedy-import -d -v /path/to/preferencefile:/root/.config/preferences/io.sked.docs.importer.pref -v /path/to/import/folder:/import teedy-import
```
